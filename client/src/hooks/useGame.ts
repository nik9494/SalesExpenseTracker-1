...
export const useGame = ({ roomId, gameId, userId }: UseGameOptions = {}) => {
  ...
  const sendTapToServer = useCallback(throttle(() => {
    if (buffer > 0 && roomId && userId && connected) {
      sendTap(roomId, userId, buffer);
      setTaps(prev => prev + buffer);
      setBuffer(0);
    }
  }, 100), [buffer, roomId, userId, connected, sendTap]);
  
  // Send reaction
  const handleSendReaction = useCallback((toPlayerId: string, reaction: string) => {
    if (!roomId || !userId || !connected) return;
  ...
  useEffect(() => {
    ...
    const unsubscribeJoin = subscribe(WsMessageType.PLAYER_JOIN, (message: WsMessage) => {
      setPlayers(prev => {
    ...
    const unsubscribeLeave = subscribe(WsMessageType.PLAYER_LEAVE, (message: WsMessage) => {
      setPlayers(prev => prev.filter(player => player.id !== message.user_id));
    });
    
    const unsubscribeRoomUpdate = subscribe(WsMessageType.ROOM_UPDATE, (message: WsMessage) => {
      setRoom(message.data.room);
      setPlayers(message.data.players || []);
    });
    
    const unsubscribeGameStart = subscribe(WsMessageType.GAME_START, (message: WsMessage) => {
      setIsStarted(true);
      setGame(message.data.game);
      setRemainingTime(message.data.duration || 60);
    });
    
    const unsubscribeGameEnd = subscribe(WsMessageType.GAME_END, (message: WsMessage) => {
      setIsFinished(true);
      setGame(message.data.game);
    ...
    const unsubscribeTap = subscribe(WsMessageType.TAP, (message: WsMessage) => {
      setPlayers(prev => 
        prev.map(player => 
    ...
    const unsubscribeReaction = subscribe(WsMessageType.PLAYER_REACTION, (message: WsMessage) => {
