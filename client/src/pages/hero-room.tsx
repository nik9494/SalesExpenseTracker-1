...
export default function HeroRoomPage() {
  ...
  const { data, isLoading } = useQuery<HeroRoomsResponse>({
    ...
    queryFn: async () => {
      const response = await apiRequest('GET', '/api/v1/rooms/hero?status=active');
      if (!response.ok) {
  ...
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.visibilityState === 'visible') {
        queryClient.invalidateQueries({ queryKey: ['/api/v1/rooms/hero'] });
      }
    };
    ...
    try {
      ...
      if (data.room) {
        ...
        if (data.room.creator_id === userId) {
          const joinResponse = await apiRequest('POST', `/api/v1/rooms/${data.room.id}/join`, {
            isObserver: true
          });
          if (joinResponse.ok) {
  ...
  const handleJoinRoom = async (roomId: string) => {
    try {
      ...
      const response = await apiRequest('POST', `/api/v1/rooms/${roomId}/join`, {
        isObserver: true
      });
